...
#commands
execute if score disrupt.cd bp.the_disruptor matches 40 run function bp:entities/boss/the_disruptor/attack/misc/disrupt
...
execute if score attack.cd bp.the_disruptor matches 100.. run function bp:entities/boss/the_disruptor/move


scoreboard players add cd bp.the_disruptor 1
execute if score attack.cd bp.the_disruptor matches ..100 if score cd bp.the_disruptor matches 20 facing entity @p eyes run function bp:entities/boss/the_disruptor/attack/misc/lazersummon
execute rotated as @e[type=minecraft:area_effect_cloud,tag=ray,nbt={Age:4}] run function bp:entities/boss/the_disruptor/attack/misc/lazer
