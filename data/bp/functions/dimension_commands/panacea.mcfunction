...
execute as @s[scores={bp.creeper=1..8}] run function bp:player/set_rotation 
execute as @s[scores={bp.creeper=0}] run function bp:player/get_rotation
execute as @s[scores={bp.creeper=0}] unless score @s bp.x_rotation = @s bp.xx_rotation unless score @s bp.z_rotation = @s bp.zz_rotation run function bp:dimension_crossing/panacea_to_overworld

effect give @s[scores={bp.overworld=2..}] jump_boost 10 255 true

##########   CHLOROCRAFTING   ##########
execute at @e[type=minecraft:item_frame,nbt={Facing:1b,Item:{id:"minecraft:bone_meal",Count:1b}}] if block ~ ~-1 ~ minecraft:crafting_table run function bp:entities/chlorocrafter
...
##########   SPORE DROPS   ##########
data merge entity @e[distance=1..5,limit=1,type=!minecraft:item,tag=!spore,sort=nearest] {DeathLootTable:"bp:panacea/sporedrop",Tags:["spore"]}
