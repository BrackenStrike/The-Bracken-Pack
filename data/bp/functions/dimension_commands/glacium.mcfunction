...
execute at @s[scores={bp.ice=400..}] if block ~ ~-1 ~ minecraft:beacon run function bp:dimension_crossing/glacium_to_overworld

# migrated to advancements
#execute if entity @s[y=-32,dy=-100] in bp:void run tp @s ~ 240 ~


execute if entity @s[scores={bp.tick=2}] run function bp:entities/glacium/glacium_entities
