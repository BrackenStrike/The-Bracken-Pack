...
execute in bp:pax align xz run tp @s ~0.5 30 ~0.5
execute at @s run forceload add ~ ~
execute at @s[gamemode=!spectator] run fill ~ 29 ~ ~ 29 ~ minecraft:sand replace water
execute at @s[gamemode=!spectator] run fill ~ ~1 ~ ~ ~0 ~ air 
advancement revoke @s only bp:dimension_crossing/faewild_to_pax
execute as @s in bp:pax run function bp:remove_forceload