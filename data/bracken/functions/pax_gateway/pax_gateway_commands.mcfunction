...
execute if entity @e[type=marker,tag=pax_scores,distance=..25,limit=1,scores={r=1,g=0,b=0,w=0}] unless entity @e[type=marker,tag=pax_scores,tag=bp.dormis_platform,distance=..25,limit=1] run function bracken:dimensions/spawn_platforms/dormis_spawn_platform
execute if entity @e[type=marker,tag=pax_scores,distance=..25,limit=1,scores={r=1,g=0,b=0,w=0}] if entity @p[distance=..30] in bracken:dormis run forceload add ~ ~
