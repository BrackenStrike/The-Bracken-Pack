...
execute if entity @s[tag=slam] unless block ~ ~-0.5 ~ air as @a[distance=..3] run function bracken:entities/boss/the_disruptor/dmg/slam

execute if entity @s[tag=slam] unless block ~ ~-0.5 ~ air run tag @s remove slam

execute if entity @s[tag=!slam] run function bracken:entities/boss/the_disruptor/move
